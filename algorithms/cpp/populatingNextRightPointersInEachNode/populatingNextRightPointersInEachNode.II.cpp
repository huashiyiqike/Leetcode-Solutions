...
public:
    ...
    static void connect(TreeLinkNode *root) {
        ...
        while(!q.empty()){
            ...
            level = q.front();q.pop();
            while(!level.empty()){
                TreeLinkNode* tmpnode = level.front();level.pop();
                if(!level.empty()){
                    tmpnode->next = level.front();
                }//else tmpnode->next = NULL;
                if(tmpnode->left != NULL) tmpq.push(tmpnode->left);
                if(tmpnode->right != NULL) tmpq.push(tmpnode->right);
            }
            if(!tmpq.empty())
                q.push(tmpq);
            while(!tmpq.empty())
