...
public class Solution {
    ...
    double findkth(int A[], int m, int B[], int n, int k) {
        ...
        int qa = Math.min(m, k / 2);
        int qb = k - qa;
        if (A[qa - 1] < B[qb - 1]) {
            return findkth(Arrays.copyOfRange(A, qa, A.length), m - qa, B, n, k - qa); // A + qa
        } else if (A[qa - 1] > B[qb - 1]) {
            return findkth(A, m, Arrays.copyOfRange(B, qb, B.length), n - qb, k - qb); // B + qb,
        } else
            return A[qa - 1];
    }
};
